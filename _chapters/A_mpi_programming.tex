...
\keyword{MPI} (Message-Passing Interface) è una specifica di interfaccia per librerie di message-passing, ideata per il modello di programmazione parallela per architetture MIMD a memoria distribuita (MIMD-DM).
MPI non è un linguaggio, ma tutte le sue operazioni sono espresse come funzioni o routine secondo i \textit{bindings} del linguaggio scelto (in questo caso, il C).

\section{Concetti Fondamentali e Inizializzazione}
Un programma MPI consiste in un insieme di processi autonomi che eseguono ciascuno i propri calcoli e che condividono tra loro i dati mediante comunicazioni inter-processo (IPC) fornite dalla libreria.
...
MPI permette di costruire \keyword{tipi derivati} a partire dai tipi base, descrivendo esplicitamente la disposizione in memoria dei dati da trasmettere. Questo evita la necessità di copiare dati non contigui in un buffer temporaneo prima dell'invio. Un tipo derivato è definito da una sequenza di coppie \((\text{tipo\_base}_i,\, \text{displacement}_i)\), dove il displacement indica l'offset in byte rispetto all'inizio del buffer.
...
\begin{note}{Semantica e Ordine}
    I messaggi sono \emph{nonovertaking}: se un mittente invia due messaggi in successione alla stessa destinazione, il secondo non può superare il primo se entrambi corrispondono alla stessa operazione di ricezione.
\end{note}

\begin{example}{Halo Exchange}
    L'\emph{halo exchange} è un pattern comunicativo tra processi paralleli in una griglia.
    Esso è richiesto, ad esempio, da un algoritmo stencil a 5 punti.\footnote{Lo stencil è una tecnica di calcolo numerico applicato su griglie di dati in cui per calcolare la nuova versione del dato \(\tilde{x}_{i,j}\) è necessario conoscere il valore dei dati adiacenti. In uno stencil a 5 punti, sono richiesti i dati \(x_{i, j-1}, x_{i+1, j}, x_{i, j+1}, x_{i-1, j}\).}
    L'\emph{halo} è una regione di memoria del processo che contiene una copia dei valori dei processi adiacenti.
    In uno stencil a 5 punti, ciascun processo ha un \emph{halo} contenente 4 dati.
    Un algoritmo iterativo basato su \emph{halo exchange} si divide in due fasi. Ciascun processo: 
    \begin{enumerate}
        \item invia il proprio dato ai processi adiacenti e richiede ai processi adiacenti i dati per aggiornare il suo halo; 
        \item effettua il calcolo.
    \end{enumerate}
    Vediamo un'implementazione con \texttt{MPI\_Sendrecv}:
    \begin{lstlisting}[language=c]
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>

#define N 10  // Dimensione del tile NxN di competenza di un processo
...
int main(int argc, char** argv) {
    ...
    // Crea topologia a griglia 2D
    MPI_Dims_create(size, 2, dims);
    MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periods, 0, &cart_comm);
    MPI_Cart_coords(cart_comm, rank, 2, coords);

    // Calcola i rank dei processi adiacenti
    MPI_Cart_shift(cart_comm, 0, 1, &up, &down);    // riga sopra/sotto
    MPI_Cart_shift(cart_comm, 1, 1, &left, &right); // colonna sinistra/destra

    /* Alloca tile (N+2)x(N+2) cosicchè la matrice contenga anche l'halo. Nota: gli angoli della matrice ([0,0], [0,N+1], [N+1][0], [N+1][N+1]) non verranno mai usati.
    double (*tile)[N+2] = malloc((N+2) * sizeof(double[N+2]));

    // Crea datatype per una colonna della matrice
    MPI_Type_vector(N, 1, N+2, MPI_DOUBLE, &col_type);
    MPI_Type_commit(&col_type);

    // Inizializza la parte interna del tile
    for (int i = 1; i <= N; i++)
        for (int j = 1; j <= N; j++)
            tile[i][j] = foo(tile[i][j]);

    /* Scambia riga superiore: invia riga 1 (la prima riga della matrice interna), riceve in riga 0 (il bordo superiore dell'halo) */
    MPI_Sendrecv(&tile[1][1], N, MPI_DOUBLE, up, 0,
                 &tile[0][1], N, MPI_DOUBLE, up, 0,
                 cart_comm, MPI_STATUS_IGNORE);

    /* Scambia riga inferiore: invia riga N (l'ultima riga della matrice interna), riceve in riga N+1 (il bordo inferiore dell'halo) */
    MPI_Sendrecv(&tile[N][1], N, MPI_DOUBLE, down, 1,
                 &tile[N+1][1], N, MPI_DOUBLE, down, 1,
                 cart_comm, MPI_STATUS_IGNORE);

    /* Scambia colonna sinistra: invia colonna 1 (la prima colonna della matrice interna), riceve in colonna 0 (il bordo sinistro dell'halo) */
    MPI_Sendrecv(&tile[1][1], 1, col_type, left, 2,
                 &tile[1][0], 1, col_type, left, 2,
                 cart_comm, MPI_STATUS_IGNORE);

    /* Scambia colonna destra: invia colonna N (l'ultima colonna della matrice interna), riceve in colonna N+1 (il bordo destro dell'halo) */
    MPI_Sendrecv(&tile[1][N], 1, col_type, right, 3,
                 &tile[1][N+1], 1, col_type, right, 3,
                 cart_comm, MPI_STATUS_IGNORE);

    // ora il tile contiene i dati dell'halo aggiornati
    // ... calcolo stencil ...
...
Altre volte, invece, la comunicazione non bloccante è d'obbligo.
...
\begin{example}{Halo Exchange II}
    Ritorniamo all'esempio precedente sull'\emph{halo exchange}. Se una parte dei dati è indipendente dall'halo, allora è inutile aspettare l'\emph{halo exchange}: meglio richiedere un trasferimento non bloccante con \texttt{MPI\_Isend} e \texttt{MPI\_recv}, calcolare e poi attendere il completamento del trasferimento con \texttt{MPI\_Waitall}, ed infine calcolare il resto.
    In questo modo, è possibile coprire una parte del costo di trasferimento.
    \begin{lstlisting}[language=C]
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char** argv) {
    ...
    MPI_Irecv(&tile[0][1], N, MPI_DOUBLE, up, 0,
              cart_comm, &requests[req_count++]);
    MPI_Isend(&tile[1][1], N, MPI_DOUBLE, up, 0,
              cart_comm, &requests[req_count++]);

    MPI_Irecv(&tile[N+1][1], N, MPI_DOUBLE, down, 1,
              cart_comm, &requests[req_count++]);
    MPI_Isend(&tile[N][1], N, MPI_DOUBLE, down, 1,
              cart_comm, &requests[req_count++]);

    MPI_Irecv(&tile[1][0], 1, col_type, left, 2,
              cart_comm, &requests[req_count++]);
    MPI_Isend(&tile[1][1], 1, col_type, left, 2,
              cart_comm, &requests[req_count++]);

    MPI_Irecv(&tile[1][N+1], 1, col_type, right, 3,
              cart_comm, &requests[req_count++]);
    MPI_Isend(&tile[1][N], 1, col_type, right, 3,
              cart_comm, &requests[req_count++]);

    // ... calcoli locali che non dipendono dall'halo ...

    MPI_Waitall(req_count, requests, statuses);

    // ... calcolo stencil che dipende dall'halo ...
...
\end{lstlisting}
La variante \texttt{MPI\_Allreduce} distribuisce il risultato a tutti i processi anziché solo al processo radice, evitando un successivo broadcast:
\begin{lstlisting}[language=C, numbers=none]
...
Per impostazione predefinita, MPI termina l'esecuzione del programma non appena si verifica un errore. 
Questo comportamento è controllato dagli \keyword{error handlers}, oggetti MPI associati ad un comunicatore, ad un file oppure ad una finestra. È possibile modificare il comportamento predefinito installando un handler personalizzato tramite:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm_set_errhandler(MPI_Comm comm, MPI_Errhandler errhandler)
\end{lstlisting}
Gli handler predefiniti disponibili sono:
\begin{itemize}
    \item \texttt{MPI\_ERRORS\_ARE\_FATAL}: l'handler predefinito; qualsiasi errore causa la terminazione immediata di tutti i processi del job MPI;
    \item \texttt{MPI\_ERRORS\_RETURN}: l'errore viene restituito come codice di ritorno della funzione MPI chiamante, lasciando al programma la responsabilità di gestirlo;
    \item \texttt{MPI\_ERRORS\_ABORT}: simile a \texttt{MPI\_ERRORS\_ARE\_FATAL}, ma la terminazione è limitata al comunicatore su cui si è verificato l'errore (disponibile da MPI-4).
\end{itemize}

Quando si utilizza \texttt{MPI\_ERRORS\_RETURN}, ogni funzione MPI restituisce \texttt{MPI\_SUCCESS} in caso di successo, oppure un codice di errore diverso da zero. È buona pratica verificare sistematicamente il valore restituito nelle applicazioni di produzione:
\begin{lstlisting}[language=C, numbers=none]
int err = MPI_Send(buf, count, MPI_DOUBLE, dest, tag, comm);
if (err != MPI_SUCCESS) {
    char error_string[MPI_MAX_ERROR_STRING];
    int len;
    MPI_Error_string(err, error_string, &len);
    fprintf(stderr, "MPI_Send fallita: %s\n", error_string);
    MPI_Abort(comm, err);
}
\end{lstlisting}
La funzione \texttt{MPI\_Error\_string} converte un codice di errore in una stringa leggibile; \texttt{MPI\_MAX\_ERROR\_STRING} è la lunghezza massima garantita per tale stringa. La funzione \texttt{MPI\_Abort} termina tutti i processi associati al comunicatore specificato, restituendo il codice di errore al sistema operativo.

\subsection{Handler personalizzati}
È possibile definire un handler di errore personalizzato tramite \texttt{MPI\_Comm\_create\_errhandler}:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm_create_errhandler(MPI_Comm_errhandler_function *fn,
                           MPI_Errhandler *errhandler)
\end{lstlisting}
La funzione di gestione deve avere la seguente firma:
\begin{lstlisting}[language=C, numbers=none]
void my_error_handler(MPI_Comm *comm, int *err, ...)
\end{lstlisting}
dove \texttt{comm} è il comunicatore su cui si è verificato l'errore ed \texttt{err} è il codice di errore. Al termine dell'uso, l'handler deve essere liberato con \texttt{MPI\_Errhandler\_free}.

L'esempio seguente installa un handler personalizzato su \texttt{MPI\_COMM\_WORLD} che stampa un messaggio diagnostico prima di terminare:
\begin{lstlisting}[language=C, numbers=none]
void my_error_handler(MPI_Comm *comm, int *err, ...) {
    char error_string[MPI_MAX_ERROR_STRING];
    int len;
    MPI_Error_string(*err, error_string, &len);
    fprintf(stderr, "[rank ?] Errore MPI: %s\n", error_string);
    MPI_Abort(*comm, *err);
}
...
\subsection{Classi di errore}
MPI raggruppa i codici di errore in \keyword{classi di errore}, ottenibili tramite:
\begin{lstlisting}[language=C, numbers=none]
MPI_Error_class(int errorcode, int *errorclass)
\end{lstlisting}
Le classi più comuni includono \texttt{MPI\_ERR\_COMM} (comunicatore non valido), \texttt{MPI\_ERR\_TYPE} (tipo di dato non valido), \texttt{MPI\_ERR\_COUNT} (conteggio negativo), \texttt{MPI\_ERR\_TAG} (tag non valido) e \texttt{MPI\_ERR\_RANK} (rank fuori dai limiti del comunicatore). Le classi consentono di reagire in modo differenziato a categorie di errori senza dover enumerare tutti i possibili codici specifici dell'implementazione.
...
\subsection{MPI\_Comm\_spawn}
La funzione fondamentale per la creazione dinamica è:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm_spawn(const char *command,
               char *argv[],
               int maxprocs,
               MPI_Info info,
               int root,
...
\begin{itemize}
    \item \texttt{command}: il percorso dell'eseguibile da avviare;
    \item \texttt{argv}: gli argomenti da passare all'eseguibile, terminati da \texttt{NULL} (oppure \texttt{MPI\_ARGV\_NULL} se non vi sono argomenti);
    \item \texttt{maxprocs}: il numero di processi figli da creare;
    \item \texttt{info}: un oggetto \texttt{MPI\_Info} che può contenere hint all'ambiente di esecuzione (es. il nodo su cui avviare i processi), oppure \texttt{MPI\_INFO\_NULL};
    \item \texttt{root}: il rank del processo nel comunicatore \texttt{comm} che effettua la chiamata (solo i parametri di questo processo sono significativi);
    \item \texttt{intercomm}: in uscita, l'inter-comunicatore che collega il processo padre ai figli;
    \item \texttt{array\_of\_errcodes}: array di lunghezza \texttt{maxprocs} che riceve il codice di errore per ciascun processo figlio; è possibile passare \texttt{MPI\_ERRCODES\_IGNORE} se non si intende verificarli.
\end{itemize}

Dal lato dei processi figli, l'inter-comunicatore verso il padre è ottenibile tramite:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm parent;
MPI_Comm_get_parent(&parent);
\end{lstlisting}
Se il processo non è stato creato dinamicamente, \texttt{parent} viene impostato a \texttt{MPI\_COMM\_NULL}.

\subsection{Inter-comunicatori}
Un \keyword{inter-comunicatore} collega due gruppi distinti di processi: il gruppo locale (quello del processo chiamante) e il gruppo remoto. A differenza degli intra-comunicatori, in un inter-comunicatore le operazioni di comunicazione avvengono sempre tra i due gruppi: un processo del gruppo locale invia a un processo del gruppo remoto e viceversa.

Le funzioni \texttt{MPI\_Comm\_rank} e \texttt{MPI\_Comm\_size} applicate ad un inter-comunicatore restituiscono rispettivamente il rank del processo nel proprio gruppo locale e la dimensione di tale gruppo. Per interrogare il gruppo remoto si usano:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm_remote_size(MPI_Comm intercomm, int *size)
MPI_Comm_remote_group(MPI_Comm intercomm, MPI_Group *group)
\end{lstlisting}

Se si desidera convertire un inter-comunicatore in un intra-comunicatore che includa entrambi i gruppi (utile per eseguire operazioni collettive tra padre e figli), si utilizza:
\begin{lstlisting}[language=C, numbers=none]
MPI_Intercomm_merge(MPI_Comm intercomm, int high, MPI_Comm *newintracomm)
\end{lstlisting}
Il parametro \texttt{high} stabilisce l'ordinamento dei rank nel nuovo comunicatore: i processi che passano \texttt{high=0} riceveranno rank più bassi di quelli che passano \texttt{high=1}.

\subsection{Esempio: padre e figli}
Il seguente esempio mostra un processo padre che crea \texttt{N\_WORKERS} processi figli, invia a ciascuno un intero da elaborare e raccoglie i risultati.

Il contenuto del file sorgente \texttt{master.c}:
\begin{lstlisting}[language=C, numbers=none]
#include "mpi.h"
#include <stdio.h>
#define N_WORKERS 4
...
int main(int argc, char *argv[]) {
    ...
    MPI_Comm intercomm;
    MPI_Comm_spawn("worker", MPI_ARGV_NULL, N_WORKERS,
                   MPI_INFO_NULL, 0, MPI_COMM_SELF,
                   &intercomm, MPI_ERRCODES_IGNORE);

    /* Invia un valore distinto a ciascun figlio */
    for (int i = 0; i < N_WORKERS; i++) {
        int data = i * 10;
        MPI_Send(&data, 1, MPI_INT, i, 0, intercomm);
    }

    /* Raccoglie i risultati */
    for (int i = 0; i < N_WORKERS; i++) {
        int result;
        MPI_Recv(&result, 1, MPI_INT, i, 0, intercomm,
                 MPI_STATUS_IGNORE);
        printf("Risultato dal figlio %d: %d\n", i, result);
    }
...
\begin{note}{}
    Il primo argomento passato alla funzione \texttt{MPI\_Comm\_spawn} è la stringa \texttt{"worker"}, che si assume sia il nome dell'eseguibile compilato a partire dal codice sorgente previsto per i worker (\texttt{worker.c}).
\end{note}

Il contenuto del file sorgente dei worker (\texttt{worker.c}):
\begin{lstlisting}[language=C, numbers=none]
#include "mpi.h"
...
int main(int argc, char *argv[]) {
    ...
    /* Riceve il valore dal padre */
    int data;
    MPI_Recv(&data, 1, MPI_INT, 0, 0, parent,
             MPI_STATUS_IGNORE);

    /* Elaborazione: raddoppia il valore */
    int result = data * 2;

    /* Restituisce il risultato al padre */
...
\subsection{MPI\_Comm\_spawn\_multiple}
Quando si desidera avviare processi che eseguono eseguibili diversi, si utilizza la variante:
\begin{lstlisting}[language=C, numbers=none]
MPI_Comm_spawn_multiple(int count,
                        ...
                        int array_of_errcodes[])
\end{lstlisting}
Questa funzione avvia \texttt{count} gruppi di processi, ciascuno con il proprio eseguibile, argomenti e numero di istanze. Il risultato è un unico inter-comunicatore verso tutti i processi figli creati, indipendentemente dall'eseguibile che eseguono.

\begin{note}{}
    La creazione dinamica di processi richiede che l'ambiente di esecuzione supporti il \keyword{dynamic process management}. Non tutte le implementazioni MPI o tutti i cluster lo abilitano per impostazione predefinita; in tali contesti è preferibile avviare in anticipo tutti i processi necessari e utilizzare \texttt{MPI\_Comm\_split} per organizzarli.
\end{note}
...
\begin{itemize}
    \item \emph{MPICH}: Sviluppata originariamente dall'\textit{Argonne National Laboratory}, è una delle implementazioni storiche più influenti. Molte altre implementazioni (comprese alcune commerciali) derivano dal codice sorgente di MPICH. È nota per la sua estrema portabilità e per essere spesso la prima a implementare le nuove versioni dello standard MPI.
    \item \emph{Open MPI}: Risultato della fusione di diversi progetti precedenti (come LAM/MPI e FT-MPI), Open MPI è oggi lo standard di fatto in molti ambienti HPC. Si distingue per un'architettura modulare che permette di caricare dinamicamente componenti ottimizzati per specifiche reti o sistemi di gestione dei processi.
\end{itemize}

\subsection{Implementazioni proprietarie}
...
\begin{itemize}
    \item \emph{Intel MPI}: Basata originariamente su MPICH, è ottimizzata specificamente per processori e interconnessioni Intel.
    \item \emph{IBM Spectrum MPI}: Utilizzata comunemente sui supercomputer IBM, offre ottimizzazioni avanzate per l'hardware Power e le GPU.
    \item \emph{NVIDIA/Mellanox HPC-X}: Una suite che include implementazioni ottimizzate per interconnessioni ad altissima velocità come InfiniBand.
\end{itemize}
