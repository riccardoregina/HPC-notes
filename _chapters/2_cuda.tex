...
\subsection{Concetti}
Nel framework CUDA, si lavora con un'astrazione dell'architettura descritta nella sezione precedente.
Infatti, in tale astrazione, ciascun CUDA core esegue un \keyword{thread}, ciascun SM esegue un \keyword{thread block}, ovvero un gruppo di thread, e l'intera GPU esegue una \keyword{grid}, ovvero un gruppo di thread block.
Sia una grid che un thread block sono tensori tridimensionali.
Ciò permette di definire una topologia secondo lo schema scelto per la suddivisione del calcolo.
All'interno di un grid, tutti i thread block devono avere la stessa forma, ovvero le stesse dimensioni, per consentire l'indirizzamento a tempo costante e memoria zero.\footnote{Altrimenti, bisognerebbe conservare, per ogni block, la sua forma, ed effettuare calcoli complessi per ricavare un indice globale per ciascun thread.}
I thread all'interno di ciascun thread block sono raggruppati in \keyword{warp}.
La dimensione di un warp è stabilmente \(32\) dalla Compute Capability \(1.0\)\footnote{La Compute Capability è un numero di versione che indica le caratteristiche, le specifiche e l'instruction set supportato da una specifica architettura CUDA. Conoscere la Compute Capability del device a disposizione è cruciale per l'ottimizzazione dei kernel su di esso.}.
I thread di un warp eseguono in \keyword{lockstep}\footnote{In realtà, a partire dalla Compute Capability \(7.0\), ciò non è più vero. È possibile, in tali device, lo scheduling asincrono dei thread.}, ovvero eseguono in modo sincronizzato, secondo la specifica SIMT, di cui abbiamo precedentemente parlato.
...
Una funzione \texttt{\_\_global}, può essere invocata solo dall'host ed esegue sul device.
Una funzione \texttt{\_\_device\_\_} può essere invocata solo dal device ed esegue sul device.
Una funzione \texttt{\_\_host\_\_} può essere invocata solo dall'host e esegue sull'host.
Se non specificato lo scope, una funzione ha scope \texttt{\_\_host\_\_}.
...
Per cominciare, la memoria centrale dell'host e la global memory della GPU sono separate ed è necessario un trasferimento di dati tra le due.
Quindi, è necessaria un'allocazione esplicita sulla global memory.
Tale allocazione è possibile, così come nel linguaggio C, in due modi: staticamente o dinamicamente.
...
I thread all'interno dello stesso block possono accedere alla shared memory, che è generalmente di un ordine di grandezza più veloce della global memory per accesso e scrittura.
...
\begin{note}{}
    \texttt{shared\_mem\_size} corrisponde alla dimensione della shared memory da allocare per tutti i block.
    Ad esempio, se vogliamo che ogni block abbia a disposizione \texttt{size} byte, allora \texttt{shared\_mem\_size} dovrà essere uguale a \texttt{size} per il numero di block.
\end{note}

%Cita https://feldmann.nyc/blog/smem-microbenchmarks
...
Accessi allo stesso banco vengono serializzati, mentre accessi a banchi diversi possono avvenire simultaneamente.
