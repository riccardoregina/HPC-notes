...
Soprattutto, quest'ultima si concentra sul numero di operazioni necessarie alla risoluzione del problema dato. 
Tuttavia, sfruttando il concetto di parallelismo, è possibile ridurre il tempo effettivo di esecuzione, proprio in termini di secondi o sottomultipli, nella risoluzione di un problema. 
...
Il parallelismo è un concetto che esiste anche solo teoricamente: un algoritmo parallelo, esattamente come accade per gli algoritmi classici, può essere valutato in via totalmente teorica, senza impiegare macchine reali. 
Ciò è di fondamentale importanza in fase di progettazione.
...
\begin{itemize}
  ...
  \item \keyword{Parallelismo spaziale}: la stessa operazione viene eseguita più volte, contemporaneamente, su più flussi di oggetti;
  \item \keyword{Parallelismo asincrono}: operazioni diverse su flussi di oggetti diversi vengono eseguiti da esecutori diversi.
\end{itemize}
...
Una CU decodifica uno \textit{stream} di istruzioni, mentre una ALU gestisce uno \textit{stream} di dati. Possiamo implementare tutti i tre tipi di parallelismo mediante un insieme di CPU e ALU.
...
\begin{note}{}
Nell'articolo in cui viene presentata la tassonomia, Flynn parla di Instruction stream, non di singola istruzione, e di Data stream, non di singolo dato. In questo contesto, la famiglia MISD assume senso: su un singolo stream di dati, vengono applicati tanti stream di istruzioni, intese come istruzioni aritmetiche. Pertanto, una macchina MISD prevede una singola CU ma con una ALU con capacità pipelining.
\end{note}

Sebbene la tassonomia di Flynn sia a fondamento della classificazione dei calcolatori, nel tempo si è passati sempre di più ad architetture ibride ed eterogenee. Si parla di \keyword{architettura ibrida} quando essa presenta più livelli di parallelismo, in cui i livelli appartengono a famiglie distinte di calcolatori paralleli secondo la tassonomia di Flynn.
...
\section{Tempo di esecuzione di un algoritmo parallelo}
...
\begin{note}{}
  Si noti che il numero di operazioni effettuate è $(\frac{n}{4} - 1)\cdot 4$, a cui bisogna, però, aggiungere il costo di distribuzione dei dati per calcolare in modo corretto il tempo di esecuzione. Tali operazioni vengono effettuate \textbf{contemporaneamente}.
\end{note}
...
\begin{note}{}
  Si potrebbe pensare che la terza strategia sia un'inutile complicazione della seconda: perché effettuare più calcoli solo per avere il risultato finale in tutti i nodi? Cerchiamo di rispondere citando i vantaggi di questo approccio:
  \begin{enumerate}
...
\subsection{Legge di Ware-Amdahl}
...
e questa espressione è chiamata \keyword{legge di Ware-Amdahl}. A questo punto, l'efficienza è espressa come
...
\subsection{Legge di Ware-Amdahl generalizzata}
...
\begin{definition}{Scalabilità forte}
  Un algoritmo si dice \keyword{fortemente scalabile} se, fissata la dimensione del problema, l'efficienza rimane costante all'aumentare del numero di processori. 
\end{definition}

\subsection{Speedup scalato}
...
Si definisce \keyword{efficienza scalata} la quantità
...
e quindi $T(1) = T_s + p(1-T_s)$. Infine, ponendo $T_s = \alpha^{'}$, si ottiene la \keyword{legge di Gustafson}:
...
\subsection{Tempo di comunicazione}
