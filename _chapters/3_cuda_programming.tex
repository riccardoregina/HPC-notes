...
Nei sistemi per l'HPC, ciascun nodo è dispone di più device e coopera con gli altri nodi mediante MPI (vedi appendice A).
In ciascun nodo, è possibile sfruttare i device in due modi:
\begin{itemize}
    \item \keyword{Multi-thread}: thread distinti dell'host gestiscono un device ciascuno e comunicano mediante OpenMP o pthread;
    \item \keyword{Multi-process}: processi distinti dell'host gestiscono un device ciascuno e comunicano mediante MPI.
\end{itemize}

Nel comunicare tra l'host (la CPU fisica, non l'host logico) e i device, il collo di bottiglia può risiedere nel bus PCIe: trasferimenti simultanei mediante il bus entrano in conflitto. Ciò può essere risolto dalla comunicazione device to device, resa possibile con NVLink\footnote{NVLink è una tecnologia di interconnessione sviluppata da NVIDIA che fornisce una connessione ad alta larghezza di banda e bassa latenza tra GPU, e tra GPU e CPU. A differenza del bus PCIe, NVLink consente trasferimenti diretti tra device senza passare attraverso l'host, riducendo significativamente i colli di bottiglia nelle comunicazioni multi-GPU.}.

\subsection{Multi-thread}
Consideriamo un esempio in cui si hanno a disposizione 4 device su un singolo nodo. Creiamo 4 thread con pthread, ricordando di effettuare il thread join alla fine. Ciascun thread seleziona il proprio device esclusivo mediante \code{cudaSetDevice()}:

\begin{lstlisting}[language=C, caption={Esempio di multi-threading con 4 device CUDA}]
#include <pthread.h>
...
void* thread_function(void* arg) {
    ...
    cudaSetDevice(thread_id);  // Ogni thread usa un device diverso
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], NULL);  // Attende il completamento
    }
...
Adesso, supponiamo di avere a disposizione due nodi distribuiti, ciascuno dei quali con 4 device. Ogni processo MPI è un nodo multi-thread come quello visto nella sottosezione precedente. La struttura gerarchica è la seguente:

\begin{itemize}
    \item \keyword{Livello inter-nodo}: comunicazione mediante MPI tra i processi sui diversi nodi;
    \item \keyword{Livello intra-nodo}: comunicazione mediante thread (pthread o OpenMP) all'interno di ciascun processo MPI, con ciascun thread che gestisce un device specifico.
\end{itemize}

\begin{lstlisting}[language=C, caption={Esempio di multi-process con MPI e multi-threading}]
#include <mpi.h>
#include <pthread.h>
#include <cuda_runtime.h>
...
void* thread_function(void* arg) {
    int thread_id = *(int*)arg;
    int global_device = thread_id;  // Device locale al nodo
    cudaSetDevice(global_device);
...
int main(int argc, char** argv) {
    ...
    // Verifica che ci siano abbastanza device
    int device_count;
    cudaGetDeviceCount(&device_count);
    if (device_count < NUM_DEVICES) {
        printf("Errore: servono almeno %d device\n", NUM_DEVICES);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    
    // Creazione dei thread per i device locali
    pthread_t threads[NUM_THREADS];
    int thread_ids[NUM_THREADS];
    ...
    // Comunicazione MPI con altri nodi
    // ... scambio dati tra processi ...
...
In questo schema, ogni processo MPI (uno per nodo) gestisce i propri 4 device mediante thread locali, mentre la comunicazione tra i nodi avviene esclusivamente mediante MPI. Questo approccio ibrido combina i vantaggi della comunicazione veloce intra-nodo (thread) con la scalabilità inter-nodo (MPI).
