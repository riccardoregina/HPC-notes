...
Verso la metà degli anni 2010, il Deep Learning dà nuovo impulso al computing: la centralità dei prodotti matriciali (GEMM) spinge NVIDIA, con l'architettura Volta (2017), ad introdurre i \emph{Tensor core}, ottimizzati per le GEMM a bassa precisione.
Negli anni 2020, l'impulso si trasforma in uno tsunami: sempre più reti neurali vengono basate su \emph{transformer}, rendendosi particolarmente adatte all'esecuzione su GPU, che diventano più che mai centrali nel campo del machine learning.
La natura sempre più GPU-bound del computing porta NVIDIA a sviluppare la Grace CPU, integrata con le GPU dell'architettura Hopper (2022) per favorire una maggiore larghezza di banda, nonchè una bassa latenza di memoria.
Producendo sia CPU che GPU, NVIDIA capisce di poter costruire in piena autonomia sistemi per l'HPC, come il rack GB200 NVL72 basato sull'architettura Blackwell (2024), nonchè sistemi consumer orientati all'AI locale come il DGX Spark.
...
\section{L'architettura CUDA}
CUDA è un'architettura ibrida che presenta tre livelli di parallelismo. Il primo, macroscopico, prevede la presenza di $p$ \keyword{Streaming Multiprocessor} (SM), che condividono tra loro la \keyword{global memory}. Quindi, a questo livello macroscopico, l'architettura è MIMD Shared Memory. Il secondo livello, intermedio, prevede che ciascun SM sia un calcolatore SIMD dotato di $Q$ \keyword{CUDA cores}. Ciascun SM ha una memoria condivisa, detta \keyword{shared memory}, posizionata sullo SM, a cui possono accedere i CUDA cores appartenenti allo SM. 
Ciascun CUDA core è dotato di una \keyword{local memory}, residente nella global memory, utilizzabile e visibile solo al CUDA core associato. 
Ciascun SM, essendo SIMD, gestisce un tipo di operazione alla volta, poiché ha una sola unità di controllo.
