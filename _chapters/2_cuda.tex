...
CUDA è un'architettura ibrida che presenta tre livelli di parallelismo. Il primo, macroscopico, prevede la presenza di $p$ core, chiamati MP, che condividono una cache ed una memoria. Quindi, a questo livello macroscopico, l'architettura è MIMD Shared Memory. Il secondo livello, intermedio, prevede che ciascun core MP sia un calcolatore SIMD dotato di $Q$ \keyword{CUDA cores}. Ciascun MP ha una memoria condivisa locale e ciascun CUDA core è dotato di una memoria locale privata. Ciascun MP, essendo SIMD, gestisce un tipo di operazione alla volta, poiché ha una sola unità di controllo.
...
L'ultimo livello di parallelismo prevede che ciascun CUDA core implementi una pipeline. La CU di un MP impiega $k$ cicli macchina per la decodifica di un'operazione, mentre un CUDA core solo $1$. Definiamo $r$ la latenza il numero di cicli necessari ad un CUDA core per andare a regime.

\begin{note}{}
Si assume $k<r$, altrimenti la pipeline non potrebbe mai andare a regime.
\end{note}
